...
int inode_delete(int inumber){
    ...
    if (inode_table[inumber].numClients > 0) {
        unlock_inode_table();
        return -2;
    }
...
void incrementNumClients(int inumber) {
    lock_inode_table();
    inode_table[inumber].numClients++;
    unlock_inode_table();
}

void decrementNumClients(int inumber) {
    lock_inode_table();
    inode_table[inumber].numClients--;
    unlock_inode_table();
}